...
export const StockTab: React.FC = () => {
    const { db, currentUser, saveDb, notify, showModal, showPinModal } = useGame();
    ...
    const handleOrder = async () => {
        ...
        const pin = await showPinModal("거래 승인 (PIN)", currentUser?.pin!);
        if (pin !== currentUser?.pin) return;

        const newDb = JSON.parse(JSON.stringify(db));
        const targetUser = newDb.users[currentUser!.name];
        const targetStock = newDb.stocks![stock.id];

        if (!targetUser.stockHoldings) targetUser.stockHoldings = {};

        if (tab === 'buy') {
            targetUser.balanceKRW -= totalAmount;
            
            const existing = targetUser.stockHoldings[stock.id] || { quantity: 0, averagePrice: 0 };
            const newQty = existing.quantity + qty;
            const newAvg = ((existing.quantity * existing.averagePrice) + totalAmount) / newQty;

            targetUser.stockHoldings[stock.id] = { quantity: newQty, averagePrice: newAvg };
            ...
            targetStock.currentPrice = Math.floor(tradePrice * (1 + 0.0001 * qty));
            notify(currentUser!.name, `${stock.name} ${qty}주 매수 체결`);
        } else {
            targetUser.balanceKRW += totalAmount;
            
            const existing = targetUser.stockHoldings[stock.id];
            existing.quantity -= qty;
            
            const profit = (tradePrice - existing.averagePrice) * qty;
            targetUser.realizedStockProfit = (targetUser.realizedStockProfit || 0) + profit;

            if (existing.quantity <= 0) {
                delete targetUser.stockHoldings[stock.id];
            }
            ...
            targetStock.currentPrice = Math.max(1, Math.floor(tradePrice * (1 - 0.0001 * qty)));
            notify(currentUser!.name, `${stock.name} ${qty}주 매도 체결`);
        }

        if (!targetStock.history) targetStock.history = [];
    ...
    return (
        <div className="flex flex-col h-full space-y-4">
            {/* Stock List Scroll - Shows owned stocks first */}
            <div className="flex overflow-x-auto gap-3 pb-2 scrollbar-hide select-none">
                <div 
                    onClick={() => setIsSungSpiView(true)}
                    className={`flex-shrink-0 min-w-[120px] p-3 rounded-xl border cursor-pointer transition-all ${isSungSpiView ? 'bg-gray-800 text-white border-gray-600' : 'bg-white dark:bg-[#1E1E1E] border-gray-200 dark:border-gray-700'}`}
                >
                ...
                {sortedStocks.map(s => {
                    ...
                    return (
                        <div 
                            ...
                            className={`relative flex-shrink-0 min-w-[140px] p-3 rounded-xl border cursor-pointer transition-all ${selectedId === s.id && !isSungSpiView ? 'bg-gray-800 text-white border-gray-600' : 'bg-white dark:bg-[#1E1E1E] border-gray-200 dark:border-gray-700'}`}
                        >
                            {myStockQty > 0 && (
