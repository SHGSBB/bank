...
export const MartProductTab: React.FC = () => {
    ...
    // ... (Existing state: name, description, variants, etc.) ...
    ...
    const handleSave = async () => {
        ...
        // Ensure to fetch current user's products again to not overwrite if outdated (though currentUser in context should be fresh enough via onValue)
        const currentProducts = currentUser!.products || {};
        const newProducts = { ...currentProducts, [prodId]: product };
        
        // Use ID for update key
        await updateUser(currentUser!.id || currentUser!.email!, { products: newProducts });
    ...
    // ... (Rest of UI helper functions: handleEdit, handleAddVariant, etc.) ...
    const handleEdit = (p: Product) => {
        ...
        else setVariants([{ name: '기본', priceKRW: p.price, priceUSD: p.priceUSD || 0 }]);
    };
    ...
    const resetForm = () => { setIsEditing(null); setName(''); setDescription(''); setImage(null); setStock('100'); setIsOnEvent(false); setDiscountPercent(''); setVariants([]); setNewVarName(''); setNewVarKRW(''); setNewVarUSD(''); setPriceDisplayMethod('min'); };

    // ... (Render logic same as previous, just ensuring handlers are wired) ...
    return (
        <div className="w-full space-y-6">
                ...
                <div className="flex flex-col gap-4">
                    ...
                    <div className="p-4 bg-gray-50 dark:bg-gray-800 rounded border border-gray-200 dark:border-gray-700">
                            ...
                            {variants.map((v, i) => (
                                <div key={i} className="flex items-center gap-2 bg-white dark:bg-black p-2 rounded border border-gray-200 dark:border-gray-700">
                                    ...
                                    <div className="flex-1">
                                        ...
                                        <span className="text-xs text-gray-500">{v.priceKRW > 0 && `₩${v.priceKRW.toLocaleString()}`} {v.priceUSD > 0 && `+ $${v.priceUSD}`}</span>
                                    </div>
                        ...
                        <div className="grid grid-cols-12 gap-2 items-end bg-gray-100 dark:bg-gray-900 p-2 rounded">
                            <div className="col-span-5"><label className="text-[10px] font-bold block mb-1">항목명</label><Input value={newVarName} onChange={e => setNewVarName(e.target.value)} className="w-full text-sm py-1" placeholder="예: 기본" /></div>
                            <div className="col-span-3"><label className="text-[10px] font-bold block mb-1">가격(₩)</label><Input type="number" value={newVarKRW} onChange={e => setNewVarKRW(e.target.value)} className="w-full text-sm py-1" placeholder="0" /></div>
                            <div className="col-span-2"><label className="text-[10px] font-bold block mb-1">가격($)</label><Input type="number" value={newVarUSD} onChange={e => setNewVarUSD(e.target.value)} className="w-full text-sm py-1" placeholder="0" /></div>
                            <div className="col-span-2"><Button onClick={handleAddVariant} className="w-full text-xs py-1.5 h-full">추가</Button></div>
                        </div>
                    </div>

                    <div>
                        <label className="text-xs font-bold block mb-1">가격 표시 방식</label>
                        <div className="flex gap-2">
                            <label className="flex items-center gap-2 cursor-pointer p-2 border rounded text-sm bg-gray-50 dark:bg-gray-800"><input type="radio" name="pdm" checked={priceDisplayMethod === 'min'} onChange={() => setPriceDisplayMethod('min')} className="accent-green-600"/>최저가</label>
                            <label className="flex items-center gap-2 cursor-pointer p-2 border rounded text-sm bg-gray-50 dark:bg-gray-800"><input type="radio" name="pdm" checked={priceDisplayMethod === 'avg'} onChange={() => setPriceDisplayMethod('avg')} className="accent-green-600"/>평균가</label>
                        </div>
                    </div>
                        ...
                        <div className="flex items-center gap-3">
                            ...
                            <div>
                                ...
                                <div className="flex items-center gap-2 text-xs text-gray-500">
                                    <button onClick={() => setStockDetailProduct(p)} className="hover:text-green-500 hover:underline font-bold">재고: {p.stock}</button>
            ...
            {stockDetailProduct && <Modal isOpen={true} onClose={() => setStockDetailProduct(null)} title={`${stockDetailProduct.name} 재고 상세`}><div className="space-y-4 text-center"><div className="p-4 bg-gray-50 dark:bg-gray-800 rounded-xl"><p className="text-sm text-gray-500 mb-1">통합 총 재고</p><p className="text-3xl font-black">{stockDetailProduct.stock?.toLocaleString()} 개</p></div><div className="text-left space-y-2"><p className="font-bold text-sm">항목별 정보</p>{stockDetailProduct.variants?.map((v, i) => (<div key={i} className="flex justify-between p-2 bg-gray-100 dark:bg-gray-700 rounded text-sm"><span>{v.name}</span><span>{v.priceKRW.toLocaleString()}원</span></div>))}</div><Button onClick={() => setStockDetailProduct(null)} className="w-full">닫기</Button></div></Modal>}
        </div>
    );
};
