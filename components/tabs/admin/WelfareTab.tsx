...
export const WelfareTab: React.FC = () => {
    ...
    // Helper: Total Asset Calculation
    ...
    const handlePayWelfare = async (userName: string, amount: number) => {
        ...
        newBank.balanceKRW -= amount;

        const date = new Date().toISOString();
        user.transactions = [...(user.transactions || []), {
            id: Date.now(), type: 'income', amount: amount, currency: 'KRW', description: '복지 지원금', date
        }];
        newBank.transactions = [...(newBank.transactions || []), {
            id: Date.now() + 1, type: 'expense', amount: -amount, currency: 'KRW', description: `${userName} 복지금 지급`, date
        }];
    ...
    const tierBuckets = sortedTiers.map(t => {
        return {
            ...t,
            citizens: citizens.filter(c => calculateTotalAsset(c) <= t.threshold)
        };
    });
    ...
    return (
        <div className="w-full">
            ...
            <div className="space-y-6 mb-8 w-full">
                {tierBuckets.map((bucket, idx) => (
                    <Card key={idx}>
                        <h4 className="font-bold mb-3 border-b pb-2">
                            자산 ₩{bucket.threshold.toLocaleString()} 이하 (지급액: ₩{bucket.amount.toLocaleString()})
                        </h4>
                        {bucket.citizens.length === 0 ? (
                            <p className="text-sm text-gray-500">해당 시민이 없습니다.</p>
                        ) : (
                            <ul className="space-y-2 max-h-40 overflow-y-auto pr-2">
                                {bucket.citizens.map(c => (
                                    <li key={c.name} className="flex justify-between items-center p-2 bg-gray-50 dark:bg-gray-800 rounded">
                                        <span className="text-sm">
                                            {c.name} <span className="text-gray-500 text-xs ml-1">(자산: ₩{calculateTotalAsset(c).toLocaleString()})</span>
                                        </span>
                                        <Button className="text-xs py-1 px-3" onClick={() => handlePayWelfare(c.name, bucket.amount)}>
                                            지급
                                        </Button>
                                    </li>
                                ))}
                ...
                {tierBuckets.length === 0 && <p className="text-center text-gray-500">설정된 복지 구간이 없습니다.</p>}
            </div>

            <Card className="border-t-4 border-gray-500 w-full">
                <h4 className="font-bold text-lg mb-4">복지 기준 설정</h4>
                <div className="space-y-3 mb-4">
                    {tiers.map((tier, i) => (
                        <div key={i} className="flex gap-2 items-center">
                            <span className="text-sm font-bold w-6">{i+1}.</span>
                            <div className="flex-1 flex items-center gap-2">
                                <span className="text-sm whitespace-nowrap">자산</span>
                                <Input 
                                    type="number" 
                                ...
                                />
                                <span className="text-sm whitespace-nowrap">이하, 지급</span>
                                <Input 
                                    type="number" 
                                    value={tier.amount} 
                                    ...
                                    className="py-1 px-2 text-sm w-full"
                                />
                                <Button variant="danger" className="py-1 px-2 text-xs" onClick={() => removeTier(i)}>X</Button>
                            </div>
                        </div>
                    ))}
                </div>
                <div className="flex gap-2">
                    <Button variant="secondary" className="w-full text-sm" onClick={addTier}>+ 구간 추가</Button>
                    <Button className="w-full text-sm" onClick={handleSaveSettings}>설정 저장</Button>
                </div>
            </Card>
        </div>
    );
