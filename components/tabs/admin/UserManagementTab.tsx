...
export const UserManagementTab: React.FC = () => {
    const { db, saveDb, showModal, showConfirm, notify, updateUser, showPinModal, currentUser } = useGame();
    ...
    const handleSave = async () => {
        ...
        newDb.users[selectedUser].balanceKRW = krw;
        newDb.users[selectedUser].balanceUSD = usd;
        newDb.users[selectedUser].type = editType;
    ...
    const handleDeleteUser = async () => {
        ...
        if (await showConfirm('정말 이 사용자를 삭제하시겠습니까?')) {
            const newDb = JSON.parse(JSON.stringify(db));
            delete newDb.users[selectedUser];
            await saveDb(newDb);
            showModal('삭제되었습니다.');
            setSelectedUser(null);
        }
    ...
    const handleApproval = async (user: User, approve: boolean) => {
        ...
        const newDb = JSON.parse(JSON.stringify(db));
        if (approve) {
            newDb.users[user.name].approvalStatus = 'approved';
            notify(user.name, '회원가입이 승인되었습니다.', true);
        } else {
            delete newDb.users[user.name];
        }
        await saveDb(newDb);
        showModal('처리되었습니다.');
    };
    ...
    const handleViewCode = async (sessionId: string) => {
        const pin = await showPinModal("코드를 확인하려면 PIN을 입력하세요.", currentUser?.pin!, currentUser?.pinLength || 4);
    ...
    const handleMoveUserToCountry = async (userName: string, countryId: string | undefined) => {
        const newDb = JSON.parse(JSON.stringify(db));
        newDb.users[userName].countryId = countryId;
        await saveDb(newDb);
    };
