...
export const formatShortPrice = (price: number) => formatSmartMoney(price, true);
...
export const MoneyInput: React.FC<React.InputHTMLAttributes<HTMLInputElement>> = (props) => <Input {...props} type="number" inputMode="numeric" />;
...
export const Modal: React.FC<{ isOpen: boolean; onClose: () => void; title?: string; children: React.ReactNode; zIndex?: number }> = ({ isOpen, onClose, title, children, zIndex = 3000 }) => {
    ...
    return (
        <div className={`fixed inset-0 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-fade-in`} style={{ zIndex }}>
            <div className="bg-white dark:bg-[#1C1C1E] rounded-[28px] w-full max-w-lg max-h-[90vh] overflow-hidden shadow-2xl animate-scale-in border border-transparent dark:border-gray-800 relative flex flex-col">
...
export const LineIcon: React.FC<{ icon: string, className?: string }> = ({ icon, className = "w-6 h-6" }) => {
    const paths: Record<string, string> = {
        ...
        'star': 'M11.049 2.927c.3-.921 1.603-.921 1.902 0l1.519 4.674',
        'plus_dashed': 'M12 9v3m0 0v3m0-3h3m-3 0H9',
        'profile': 'M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z',
        'id_card': 'M3 10h18M7 15h1m4 0h1m-7 4h12a2 2 0 002-2V7',
        'security': 'M9 12l2 2 4-4',
...
export const PinModal: React.FC<{ resolver: any; setResolver: any }> = ({ resolver, setResolver }) => {
    ...
    useEffect(() => { if (pin.length === (resolver.pinLength || 4)) handleComplete(pin); }, [pin]);
    return (
        <div className="fixed inset-0 z-[6000] flex items-center justify-center bg-black/60 backdrop-blur-xl animate-fade-in p-4">
            <div className={`bg-white dark:bg-[#1C1C1E] rounded-[32px] p-8 w-full max-w-[340px] shadow-2xl animate-slide-up border border-white/10 relative ${isError ? 'animate-shake border-red-500' : ''}`}>
                ...
                <div className="grid grid-cols-3 gap-4">
                    {[1, 2, 3, 4, 5, 6, 7, 8, 9].map(num => <button key={num} onClick={() => setPin(p => p + num)} className="h-16 rounded-[18px] bg-gray-100 dark:bg-gray-800 text-2xl font-bold active:scale-95">{num}</button>)}
                    ...
                    <button onClick={() => setPin(p => p + '0')} className="h-16 rounded-[18px] bg-gray-100 dark:bg-gray-800 text-2xl font-bold">0</button>
                    <button onClick={() => setPin(p => p.slice(0, -1))} className="h-16 rounded-[18px] flex items-center justify-center text-gray-500"><LineIcon icon="arrow-left" className="w-6 h-6" /></button>
                </div>
            </div>
        </div>
    );
...
// Fix: Added missing PieChart component
...
// Fix: Added missing ToastContainer component
export const ToastContainer: React.FC = () => {
    const { toasts, removeToast } = useGame();
    return (
        <div className="fixed top-6 right-6 z-[9999] flex flex-col gap-3 max-w-sm w-full pointer-events-none">
            {toasts.map(t => (
                <div key={t.id} className="pointer-events-auto bg-white dark:bg-[#1C1C1E] border-l-4 border-green-500 p-4 rounded-xl shadow-2xl animate-slide-in flex justify-between items-start gap-4">
                    <div>
                        <p className="font-bold text-sm">{t.message}</p>
                        <p className="text-[10px] text-gray-400 mt-1">{new Date(t.timestamp).toLocaleTimeString()}</p>
                    </div>
...
// Fix: Added missing FileInput component
